...
class ContentStorage extends SQLiteStorage
{
  getContent( filter )
  {
    ...
    if(filter.link) {
      where += ' AND ref IN (SELECT DISTINCT content FROM content WHERE link=?)';
      params.push(filter.link);
    }
...
// console.log(filter);
  ...
  saveContent( content )
  {
    ...
    //     stat.finalize();
    //   })
    this._saveObject('content', content);
  }

  saveContentXRef( contentXRef )
  {
    // return this._getInsertStatement('content_xref', contentXRef)
    //   .then(stat => {
    //     stat.run(Object.values(content));
    //     stat.finalize();
    //   })
    this._saveObject('content_xref', contentXRef);
  }
...
class ContentXRef
{
  ...
  {
    Object.getOwnPropertyNames(this).every((k,i)=>this[k]=arguments[i]);
  }
}
...
class Content
{
  ...
  static getContentByRef( contentRef )
  {
   ...
   return st.getContent( new ContentFilter({ref: contentRef}) );
  }

  static getContentForLink( url )
  {
    const st = new ContentStorage();
    return st.getContent( new ContentFilter({link: url}) );
  }
  ...
  static getContent()
  {
   ...
   return st.getContent(new ContentFilter());
  }
  ...
  static saveContentXRef( parentRef, ref, sorder )
  {
    ...
    return st.saveContentXRef(xref);
  }
...
Content.registerContentType('article', Link);

class Article extends Content {}
Content.registerContentType('link', Article);
