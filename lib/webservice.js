...
class WebService
{
  ...
  getParam( key, store )
  {
    ...
    if(store == 'url') return this.url[key] || null;
    if(store == 'header') return this.req.headers[key] || null;
  }
  ...
  setCookie(name, value)
  {
    ...
    const domain = this.req.host;
    const cookie = '; Path=/; Domain: '+domain+'; Secure; HttpOnly';
    this.res.setHeader('Set-Cookie', name + '=' + value + cookie );
    return;
  }
  ...
  {
    const m = new Map();
    this.parseHeader('cookie:'+this.req.headers['cookie'], (k,v) => {
      m.set(k,v);
    });
    return m;
  }

  parseHeader( header, callback )
  {
  ...
  async parseMultipartFormData()
  {
    ...
    while( (i = buf.indexOf(newline, offSet)) && i >= 0 )
    {
      ...
      // else if(parseHeaders) parseHeader(sub.toString(), headers);
        ...
        this.parseHeader(sub.toString(),(k,v)=>{headers.set(k,v)});
