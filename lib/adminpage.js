...
class AdminPage extends WebPage
{
  ...
  async run()
  {
    ...
    this.data.admin = {
      nav: ['dashboard','settings','content','plugins']
    };
    ...
    // Content
      ...
      if(plugin && this.data.plugins.has(plugin)) {
        ...
        this.template = this.data.plugin.path + '/admin.pug';
        this.data.service = System.require(plugin, 'plugin');

        this.data.service.handleRequest();
      } else {
        this.template = 'admin';
      }
    }
