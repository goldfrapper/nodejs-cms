...
class SQLiteStorage {
  ...
  {
    const tuples = Object.keys(params).map(e => e+'=?');
    const sql = `SELECT * FROM ${table} WHERE ${tuples.join(' AND ')}`;

    return this._getPromise('each', sql, Object.values(params))
    .then(row => {
  ...
  _logQuery(query, time) {
    if(typeof query != 'string') return;
    console.log('\x1b[0;32m[DB] %sms %s\x1b[0m',
      time, query.replace(/\s+/ig,' '));
      // .substring(0,60)
  }
  ...
  async _getConnection() {
    ...
    return new Promise((resolve, reject)=>{
      if(typeof SQLiteStorage.#_db !== 'object') {
        ...
        // Development: Trace
        SQLiteStorage.#_db.on('profile',(sql, time) => {
          this._logQuery(sql, time);
        });
